  ...
  return (
    <Box sx={{ mt: 2, p: 2, bgcolor: "#f8fafc", borderRadius: 2, border: "1.5px solid #e2e8f0" }}>
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, mt: 1 }}>
        ...
        <Button size="small" onClick={refresh} sx={{ minWidth: 0, color: "#64748b", fontSize: 18, p: 0.5 }} title="New question">
          ↻
        </Button>
...
export default function HospitalLogin() {
  ...
  const handleLogin = async () => {
    ...
    try {
      setLoading(true);
      const res = await axios.post(`${BASE_API}/api/hospital/login`, form);

      const facility = res.data.facility;
      const token    = res.data.token;

      loginAs("hospital", res.data.token);
      localStorage.setItem("hospitalUser",  JSON.stringify(facility));
      localStorage.setItem("hospitalToken", token);
      localStorage.setItem("activeRole",    "hospital");

      login(
        token,
        { name: facility.facilityName, email: facility.email, type: "hospital" },
        "hospitalToken",
        "hospitalUser"
      );
  ...
  const sendOtp = async () => {
    try {
      await axios.post(`${BASE_API}/api/hospital/forgot-password`, { email: forgotData.email });
  ...
  const resetPassword = async () => {
    ...
    try {
      await axios.post(`${BASE_API}/api/hospital/reset-password`, forgotData);
      ...
      <Container maxWidth="sm">
        <Paper elevation={4} sx={{ p: 4, borderRadius: 3 }}>
          <Box textAlign="center" mb={4}>
            <Avatar sx={{ width: 70, height: 70, bgcolor: "#0077a3", margin: "0 auto 16px" }}>
              <LocalHospital sx={{ fontSize: 35 }} />
            </Avatar>
            <Typography variant="h4" fontWeight={700} color="#0077a3">Hospital Login</Typography>
            <Typography variant="body2" color="text.secondary" mt={1}>
              Access your hospital facility dashboard
            </Typography>
          </Box>
          ...
          <Button fullWidth variant="contained" size="large"
            ...
            sx={{ mt: 3, py: 1.5, fontWeight: "bold", bgcolor: "#0077a3", "&:hover": { bgcolor: "#005f8a" }, "&:disabled": { bgcolor: "#94a3b8" } }}>
            {loading ? "Logging in..." : "Login"}
          </Button>

          <Box textAlign="center" mt={2}>
            <Link component="button" variant="body2" onClick={() => setForgotOpen(true)}
              sx={{ color: "#0077a3", cursor: "pointer", textDecoration: "none" }}>
              Forgot Password?
            </Link>
          </Box>
          ...
          <Box textAlign="center">
            <Typography variant="body2" color="text.secondary">
              New hospital?{" "}
              <Link href="/register/hospital" sx={{ color: "#0077a3", fontWeight: 600 }}>Register Here</Link>
            </Typography>
          </Box>
        </Paper>
      </Container>
      ...
      <Dialog open={forgotOpen} onClose={() => setForgotOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          ...
          <Button fullWidth variant="outlined"
            sx={{ mt: 2, color: "#0077a3", borderColor: "#0077a3" }} onClick={sendOtp}>
            Send OTP
          </Button>
          ...
          <Button fullWidth variant="contained" onClick={resetPassword}
            ...
            sx={{ mt: 2, bgcolor: "#0077a3", "&:hover": { bgcolor: "#005f8a" }, "&:disabled": { bgcolor: "#94a3b8" } }}>
            Reset Password
          </Button>
        </DialogContent>
      </Dialog>
