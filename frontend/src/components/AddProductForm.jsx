  ...
  return (
    ...
    >
      ...
      <Typography variant="caption" color="text.secondary">
        {type === "image"
          ? "Drag & drop • 800×800px recommended"
          : "Upload video file"}
      </Typography>
      <input
        type="file"
...
const SectionsManager = ({ sections, onChange }) => (
  <Box>
    <Typography variant="subtitle2" fontWeight={700} mb={1}>
      Product Sections / Collections
    </Typography>
    <Alert severity="info" sx={{ py: 0.5, mb: 1.5, fontSize: 12 }}>
      Products with the same section tag appear together in "Customers Also
      Viewed".
    </Alert>
...
const OffersManager = ({ offers, onChange }) => {
  ...
  return (
    <Box>
      <Typography variant="subtitle2" fontWeight={700} mb={1}>
        Offers with Icons (per user role)
      </Typography>
      <Box
        display="flex"
        gap={1}
...
const CouponManager = ({ coupons, onChange }) => {
  ...
  return (
    <Box>
                ...
                <TableRow key={i} hover>
                  ...
                  <TableCell>
                    ...
                    <IconButton
                      ...
                      color="error"
                      onClick={() =>
                        onChange(coupons.filter((_, j) => j !== i))
                      }
                    >
      ...
      >
        <DialogTitle>
          {editing !== null ? "Edit Coupon" : "New Coupon"}
        </DialogTitle>
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            <Box display="flex" gap={1}>
              ...
              />
              <IconButton onClick={generateCode} title="Auto-generate">
                <QrCode2 />
              </IconButton>
            </Box>
            ...
            <TextField
              label={`Discount ${form.type === "percent" ? "%" : "₹"} *`}
              fullWidth
              size="small"
              type="number"
              value={form.discount}
              ...
              <Box display="flex" flexWrap="wrap" gap={0.75}>
                {ROLES_LIST.filter((r) => r.value !== "all").map((role) => (
                  <Chip
                    ...
                    size="small"
                    color={
                      form.roles?.includes(role.value) ? "primary" : "default"
                    }
                    variant={
                      form.roles?.includes(role.value) ? "filled" : "outlined"
                    }
                    onClick={() => {
...
function ProductPicker({
  ...
  searchEndpoint = "/api/admin/products/search",
}) {
  ...
  useEffect(() => {
    ...
    fetch(
      `${API_BASE}${searchEndpoint}?q=${encodeURIComponent(dq)}&limit=8`,
      {
        headers: {
      ...
      .then((d) =>
        setResults(
          (d.products || d.data || []).filter((p) => !exclude.includes(p._id)),
        ),
      )
      .catch(() => setResults([]))
            ...
            >
              ...
              <Box flex={1} minWidth={0}>
                ...
                <Typography variant="caption" color="text.secondary" noWrap>
                  {p.genericCompositions || p.manufacturer || ""} · MRP ₹
                  {p.mrp || "—"}
                </Typography>
              </Box>
              <CheckCircleIcon
                sx={{ fontSize: 16, color: "#1565c0", opacity: 0.4 }}
              />
            </Box>
          ))}
        </Paper>
...
function ComboDialog({
  ...
  useEffect(() => {
    if (open) {
      if (initial) {
        ...
        // Pre-fill Product 1 with the current product being edited
        const prefilled = currentProduct
          ? [
              {
  ...
  const totalMrp = form.products.reduce(
    (s, p) => s + (parseFloat(p.mrp) || 0),
    0,
  );
    ...
    >
      ...
      >
        <Box display="flex" alignItems="center" gap={1}>
          ...
          {initial ? "Edit Combo" : "Create Product Combo"}
        </Box>
      ...
      <DialogContent>
        <Stack spacing={2.5} sx={{ mt: 1.5 }}>
          <TextField
            label="Combo Name (optional)"
            size="small"
            fullWidth
            value={form.name}
            onChange={(e) => setF("name", e.target.value)}
            placeholder="e.g. Immunity Starter Pack…"
          />
          ...
          <Box>
            ...
            <Alert severity="info" sx={{ py: 0.5, mb: 1.5, fontSize: 12 }}>
              {currentProduct
                ? `"${currentProduct.brandName}" is pre-filled as Product 1. Search for Product 2.`
                : "Search and pick exactly 2 products."}
            </Alert>
              ...
              {form.products.map((p, i) => (
                <Paper
                  ...
                  sx={{
                    ...
                    p: 1.5,
                    borderRadius: 2,
                    borderColor: i === 0 ? "#bfdbfe" : "#bbf7d0",
                    bgcolor: i === 0 ? "#f0f7ff" : "#f0fdf4",
                  }}
                >
                  <Avatar
                    src={p.images?.[0]?.url}
                  ...
                  <Chip
                    label={i === 0 ? "Product 1 (Current)" : "Product 2"}
                    size="small"
                    color={i === 0 ? "primary" : "success"}
                  />
                  {/* Only allow removing if it's not the pre-filled current product */}
                  {!(
                    i === 0 &&
                    currentProduct &&
                ...
                <Box>
                  ...
                  >
                    {form.products.length === 0
                      ? "Add Product 1"
                      : "Search for Product 2 to pair with this product"}
                  </Typography>
                  <ProductPicker
                    onSelect={addProduct}
                    exclude={form.products.map((p) => p._id)}
                    label={`Search for product ${form.products.length + 1}…`}
                    authTokenKey={authTokenKey}
                    searchEndpoint={productSearchPath}
                  />
                </Box>
              )}
            </Stack>
          ...
          <Box>
            ...
            {comboPrice > 0 && comboPrice >= totalMrp && totalMrp > 0 && (
              <Alert severity="warning" sx={{ mt: 1, py: 0.5, fontSize: 12 }}>
                Combo price should be less than individual total.
              </Alert>
            )}
          </Box>
          ...
          <FormControlLabel
            ...
            label={
              <Typography variant="body2" fontWeight={600}>
                {form.active !== false
                  ? "Active — visible on product pages"
...
function ProductComboBuilder({
  ...
  const handleSave = (form) => {
    if (editing) {
      onChange(combos.map((c) => (c.id === form.id ? form : c)));
    } else {
      onChange([...combos, form]);
    }
    setDialogOpen(false);
  };
    ...
    <Box>
      ...
      >
        <Box>
          ...
          <Typography variant="caption" color="text.secondary">
            Pick any 2 products and set a discounted combo price. Shows on both
      ...
      <Alert severity="info" sx={{ py: 0.5, mb: 2, fontSize: 12 }}>
        Each combo links exactly <strong>2 products</strong>. It will appear on{" "}
        <em>both</em> product detail pages automatically.
        {currentProduct && (
          <strong>
            {" "}
            "{currentProduct.brandName}" will be auto-added as Product 1.
          </strong>
        )}
      </Alert>
      ...
      {combos.length === 0 ? (
        ...
        >
          ...
          <Typography
            ...
            mb={1.5}
          >
            Create your first product bundle to boost average order value.
          </Typography>
          <Button
            variant="outlined"
            size="small"
        ...
        combos.map((combo) => {
          const p1 = combo.products?.[0];
          const p2 = combo.products?.[1];
          const totalMrp =
            (parseFloat(p1?.mrp) || 0) + (parseFloat(p2?.mrp) || 0);
          const cp = parseFloat(combo.comboPrice) || 0;
          const saving = totalMrp - cp;
          const discPct =
            totalMrp > 0 ? Math.round((saving / totalMrp) * 100) : 0;
          ...
          return (
            <Paper
              ...
              sx={{
                ...
                mb: 2,
                bgcolor: combo.active === false ? "#fafafa" : "white",
                opacity: combo.active === false ? 0.7 : 1,
                position: "relative",
                "&:hover": { boxShadow: 3 },
              }}
            >
                ...
                {[p1, p2].map((p, i) =>
                  p ? (
                    <React.Fragment key={i}>
                      {i === 1 && (
                        <SwapIcon sx={{ color: "#9ca3af", fontSize: 24 }} />
                      )}
                      <Box
                        sx={{
                          display: "flex",
      ...
      <ComboDialog
        ...
        authTokenKey={authTokenKey}
        productSearchPath={productSearchPath}
      />
    </Box>
  );
}
...
function RecommendationsManager({
  ...
  const addCert = () => {
    ...
    onCertChange([
      ...
      {
        id: Date.now(),
        label: certForm.label,
        issuedBy: certForm.issuedBy,
        year: certForm.year,
        image: certForm.imagePreview,
      },
    ]);
    setCertForm({
      label: "",
    ...
    <Stack spacing={3}>
      ...
      <Box>
        <Box display="flex" alignItems="center" gap={1} mb={1.5}>
          ...
          <Chip
            label={`${recommendations.length} added`}
            size="small"
            color="success"
          />
        </Box>
        <Alert severity="info" sx={{ py: 0.5, mb: 2, fontSize: 12 }}>
          Add doctor or expert recommendations with a photo. These appear on the
          product detail page as trust signals.
        </Alert>
  ...
  <Box
    component="img"
    src={rec.image}
    sx={{ width: 48, height: 48, borderRadius: "50%", objectFit: "cover", border: "2px solid #e8f5e9", flexShrink: 0 }}
    onError={(e) => { e.target.style.display = "none"; }}
  />
) : (
  <Avatar sx={{ width: 48, height: 48, bgcolor: "#e8f5e9" }}>
    <RecommendIcon color="success" />
  </Avatar>
)}
                <Box flex={1} minWidth={0}>
                ...
                <IconButton
                  ...
                  color="error"
                  onClick={() =>
                    onRecommendChange(recommendations.filter((_, j) => j !== i))
                  }
                >
                  <DeleteIcon fontSize="small" />
                </IconButton>
            ...
            <Grid item xs={12} sm={9}>
              <Stack spacing={1.5}>
                <TextField
                  ...
                  fullWidth
                  label="Name / Title *"
                  placeholder="e.g. Dr. Anjali Sharma, MBBS"
                  value={recForm.label}
                  onChange={(e) =>
                    setRecForm((f) => ({ ...f, label: e.target.value }))
                  }
                />
                <TextField
                  size="small"
                  ...
                  placeholder="e.g. Senior Cardiologist, AIIMS Delhi"
      ...
      <Box>
        <Box display="flex" alignItems="center" gap={1} mb={1.5}>
          ...
          <Chip
            label={`${certifications.length} added`}
            size="small"
            color="primary"
          />
        </Box>
        <Alert severity="info" sx={{ py: 0.5, mb: 2, fontSize: 12 }}>
          Add certifications like ISO, FSSAI, GMP, CDSCO, Ayush etc. with their
          logo/badge image.
        </Alert>
  ...
  <Box
    component="img"
    src={cert.image}
    sx={{ width: 48, height: 48, objectFit: "contain", border: "1px solid #e3f2fd", borderRadius: 1, flexShrink: 0, bgcolor: "white", p: 0.5 }}
    onError={(e) => { e.target.style.display = "none"; }}
  />
) : (
  <Avatar sx={{ width: 48, height: 48, bgcolor: "#e3f2fd" }}>
    <VerifiedIcon color="primary" />
  </Avatar>
)}
                <Box flex={1} minWidth={0}>
                ...
                <IconButton
                  ...
                  color="error"
                  onClick={() =>
                    onCertChange(certifications.filter((_, j) => j !== i))
                  }
                >
                  <DeleteIcon fontSize="small" />
                </IconButton>
            ...
            <Grid item xs={12} sm={9}>
              <Stack spacing={1.5}>
                <TextField
                  ...
                  placeholder="e.g. ISO 9001:2015, FSSAI, GMP Certified"
                  value={certForm.label}
                  onChange={(e) =>
                    setCertForm((f) => ({ ...f, label: e.target.value }))
                  }
                />
                <Box display="flex" gap={1.5}>
                  ...
                  <TextField
                    ...
                    value={certForm.year}
                    onChange={(e) =>
                      setCertForm((f) => ({ ...f, year: e.target.value }))
                    }
                  />
                </Box>
                <Button
...
const CombinationBuilder = ({ combinations, onChange }) => {
  ...
  useEffect(() => {
    if (combinations?.length > 0 && attrs.length === 0) {
      ...
      if (firstCombo?.attributes) {
        const reconstructed = Object.entries(firstCombo.attributes).map(
          ([name]) => ({
            name,
            values: [
              ...new Set(
                combinations.map((c) => c.attributes?.[name]).filter(Boolean),
              ),
            ],
          }),
        );
    ...
    onChange(
      result.map((combo) => {
        const existing = combinations?.find((c) =>
          newAttrs.every(
            (a) => (c.attributes?.[a.name] || c[a.name]) === combo[a.name],
          ),
        );
        return {
          attributes: combo,
  ...
  return (
    <Box>
      ...
      <Alert severity="info" sx={{ py: 0.5, mb: 1.5, fontSize: 12 }}>
        Add attributes like "Type: Pack Of 4, Pack Of 6" — all combinations
        generate automatically. You can add multiple values per attribute.
      </Alert>
        ...
        <Box display="flex" gap={1} flexWrap="wrap" alignItems="flex-end">
          ...
          {attrs.length > 0 && (
            <Button
              ...
              sx={{ mb: 2.5 }}
              onClick={() => {
                if (window.confirm("Clear all attributes?")) {
                  setAttrs([]);
                  onChange([]);
                }
              }}
      ...
      {combinations?.length > 0 && (
        <>
          ...
          >
            <Typography variant="caption" color="text.secondary">
              {combinations.length} combination(s) — fill in price & stock for
              each
            </Typography>
          </Box>
          <TableContainer
                  ...
                  <TableRow key={idx} hover>
                    ...
                    <TableCell>
                      <TextField
                        ...
                        value={c.stock}
                        onChange={(e) =>
                          updateCombo(idx, "stock", e.target.value)
                        }
                        sx={{ width: 80 }}
                      />
                    </TableCell>
                    <TableCell>
                      <TextField
                        ...
                        value={c.sku}
                        onChange={(e) =>
                          updateCombo(idx, "sku", e.target.value)
                        }
                        placeholder="Auto"
                        sx={{ width: 120 }}
                      />
                        ...
                        <label>
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              reader.onload = (ev) =>
                                updateCombo(idx, "image", ev.target.result);
    ...
    <Box>
      ...
      >
        <Typography variant="subtitle2" fontWeight={700}>
          Author / Contributor Details (1mg-style)
        </Typography>
        <Button
          variant="contained"
          size="small"
      ...
      <Alert severity="info" sx={{ py: 0.5, mb: 1.5, fontSize: 12 }}>
        These appear in the product detail page sidebar like 1mg.
      </Alert>
      {authors.length > 0 && (
          ...
          {authors.map((a, i) => (
            ...
            >
              ...
              <Box flex={1}>
                ...
                {a.designation && (
                  <Typography variant="caption">{a.designation}</Typography>
                )}
              </Box>
              <Box>
      ...
      >
        <DialogTitle>
          {editing !== null ? "Edit Author" : "Add Author"}
        </DialogTitle>
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            <FormControl size="small" fullWidth>
            ...
            <Box>
              ...
              >
                ...
                <Button size="small" startIcon={<AddIcon />} onClick={addEdu}>
                  Add
                </Button>
              </Box>
              {(form.education || []).map((e, i) => (
                <Box key={i} sx={{ display: "flex", gap: 1, mb: 1 }}>
                  ...
                  <TextField
                    ...
                    value={e.institution}
                    onChange={(ev) =>
                      updateEdu(i, "institution", ev.target.value)
                    }
                    sx={{ flex: 2 }}
                  />
                  <TextField
            ...
            <Box>
              ...
              >
                ...
                <Button size="small" startIcon={<AddIcon />} onClick={addExp}>
                  Add
                </Button>
              </Box>
              {(form.experience || []).map((ex, i) => (
                <Box key={i} sx={{ display: "flex", gap: 1, mb: 1 }}>
                  ...
                  <TextField
                    ...
                    onChange={(ev) =>
                      updateExp(i, "organization", ev.target.value)
                    }
                    sx={{ flex: 2 }}
                  />
                  <TextField
...
const ReviewManagement = ({ reviews, onChange }) => {
  ...
  const save = () => {
    if (editing !== null) {
      ...
      u[editing] = form;
      onChange(u);
    } else onChange([...reviews, { ...form, id: Date.now() }]);
    setOpen(false);
    setEditing(null);
    setForm({
      user: "",
  ...
  return (
    <Card variant="outlined">
      <CardContent>
                  ...
                  <TableRow key={i} hover>
                    ...
                    <TableCell>
                      ...
                      <IconButton
                        ...
                        color="error"
                        onClick={() =>
                          onChange(reviews.filter((_, j) => j !== i))
                        }
                      >
                        <DeleteIcon fontSize="small" />
                      </IconButton>
        ...
        >
          <DialogTitle>
            {editing !== null ? "Edit Review" : "Add Review"}
          </DialogTitle>
          <DialogContent>
            <Stack spacing={2} sx={{ mt: 1 }}>
              <TextField
...
const PharmaProductForm = ({
  ...
  const [fd, setFd] = useState({
    ...
    mrp: "",
    amp: "",
    totalCostOfStores: "0.00",
    totalActualValue: "0.00",
    b2cDiscount: "",
    saleRatePTR: "0.00",
    totalSaleValue: "0.00",
    b2bDiscount: "",
    ...
    productRating: 4.5,
    doctorFeedbacks: [],
    currentStatus1: "active",
    currentStatus2: "appear",
    topReviewFromIndia: "",
    isOTC: true,
    marketerName: "",
    marketerAddress: "",
  ...
  const loadCategories = useCallback(async () => {
    const token = getManagedProductToken(authTokenKey);
    for (const url of [
      `${API_BASE}/api/category/all`,
      `${API_BASE}/api/categories/all`,
    ]) {
      try {
  ...
  (async () => {
    try {
      const res = await fetch(`${API_BASE}/api/sections/all`, {
        headers: {
          "Content-Type": "application/json",
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
        },
  ...
  const loadFranchiseZones = useCallback(async () => {
    if (hideFranchiseZoneField || fixedFranchiseZoneId) {
      setFranchiseZones([]);
      return;
    }
    ...
    try {
      ...
      setFranchiseZonesLoading(false);
    }
  ...
  useEffect(() => {
    if (fixedFranchiseZoneId) {
      setSelectedFranchiseZone(fixedFranchiseZoneId);
    }
  }, [fixedFranchiseZoneId]);
  ...
  useEffect(() => {
    ...
    requestClient
      .get(getProductPath(editProductId))
      .then((res) => {
        ...
        setFd((prev) => ({
          ...
          batchDateEffect: p.dateOfEffect || "",
          expiryDate: p.expiryDate || "",
          caseBoxPackage: p.caseBoxPackage || "",
          packageType: p.packageType || "",
          productWeight: p.productWeight || "",
          minRateFixed: p.minRateFixed || "",
          mrp: p.mrp || "",
          amp: p.amp || "",
          b2cDiscount: p.discountB2C || "",
          b2bDiscount: p.discountB2B || "",
          wholesaleSaleRate: p.wholesaleSaleRate || p.rateWholesale || "",
          casePackPrice: p.casePackPrice || "",
          hospitalDiscount: p.discountHospital || "",
          pharmacyDiscount: p.discountPharmacy || "",
          vendorDiscount: p.discountVendor || "",
          franchiseDiscount: p.discountFranchise || "",
          manufacturerDiscount: p.discountManufacturer || "",
          gstDateEffect: p.gstDateEffect || "",
          ...
          moreInformation: p.shortDescription || "",
          disclaimer: p.disclaimer || "",
          productRating: p.rating || 4.5,
          doctorFeedbacks: p.doctorFeedbacks || [],
          currentStatus1: p.statusActive || "active",
          currentStatus2: p.statusAppear || "appear",
        ...
        if (p.gst_igst) {
          ...
          setGstValues({
            igst: String(p.gst_igst),
            cgst: String(p.gst_cgst || 0),
            sgst: String(p.gst_sgst || 0),
          });
        }
        if (p.category?._id) setSelectedMainCat(p.category._id);
        if (p.subCategory?._id) setSelectedSubCat(p.subCategory._id);
        setSelectedFranchiseZone(
          p.franchiseZoneId?._id || p.franchiseZoneId || "",
        );
  ...
  useEffect(() => {
    ...
    setFd((p) => ({
      ...
      saleRatePTR: fmt(mrp * (1 - n(fd.b2cDiscount) / 100)),
      totalSaleValue: fmt(mrp * (1 - n(fd.b2cDiscount) / 100) * stocks),
      totalBusinessSaleValue: fmt(mrp * (1 - n(fd.b2bDiscount) / 100) * stocks),
      totalWholesaleValue: fmt(n(fd.wholesaleSaleRate) * stocks),
      hospitalSaleRate: fmt(mrp * (1 - n(fd.hospitalDiscount) / 100)),
      hospitalTotalValue: fmt(
        mrp * (1 - n(fd.hospitalDiscount) / 100) * stocks,
      ),
      pharmacySaleRate: fmt(mrp * (1 - n(fd.pharmacyDiscount) / 100)),
      pharmacyTotalValue: fmt(
        mrp * (1 - n(fd.pharmacyDiscount) / 100) * stocks,
      ),
      vendorRate: fmt(mrp * (1 - n(fd.vendorDiscount) / 100)),
      franchiseRate: fmt(mrp * (1 - n(fd.franchiseDiscount) / 100)),
      manufacturerRate: fmt(mrp * (1 - n(fd.manufacturerDiscount) / 100)),
    }));
  }, [
    fd.totalStocks,
    fd.mrp,
    ...
    fd.b2cDiscount,
    fd.b2bDiscount,
    fd.wholesaleSaleRate,
    fd.hospitalDiscount,
    fd.pharmacyDiscount,
    fd.vendorDiscount,
    fd.franchiseDiscount,
    fd.manufacturerDiscount,
  ]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (isBulkManufacturingDesk && sections.length === 0) {
      errors.push("At least one Website Section");
    }
    if (errors.length) {
      show(`Required: ${errors.join(", ")}`, "error");
      return;
    }
    setSubmitting(true);
    try {
      ...
      const payload = {
        ...
        batchDateEffect: fd.batchDateEffect,
        expiryDate: fd.expiryDate,
        minRateFixed: fd.minRateFixed,
        mrp: fd.mrp,
        amp: fd.amp,
        saleRatePTR: fd.saleRatePTR,
        wholesaleSaleRate: fd.wholesaleSaleRate,
        hospitalSaleRate: fd.hospitalSaleRate,
        pharmacySaleRate: fd.pharmacySaleRate,
        vendorRate: fd.vendorRate,
        franchiseRate: fd.franchiseRate,
        manufacturerRate: fd.manufacturerRate,
        ...
        b2cDiscount: fd.b2cDiscount,
        b2bDiscount: fd.b2bDiscount,
        hospitalDiscount: fd.hospitalDiscount,
        pharmacyDiscount: fd.pharmacyDiscount,
        vendorDiscount: fd.vendorDiscount,
        franchiseDiscount: fd.franchiseDiscount,
        manufacturerDiscount: fd.manufacturerDiscount,
        b2bRate: String(n(fd.mrp) * (1 - n(fd.b2bDiscount) / 100)),
        hospitalRate: fd.hospitalSaleRate,
        pharmacyRate: fd.pharmacySaleRate,
        gst: gstValues,
        ...
        certifications,
        tags: fd.tags,
        showInCategories: Array(9).fill(false),
        description: fd.description,
        moreInformation: fd.moreInformation,
        disclaimer: fd.disclaimer,
        currentStatus1: fd.currentStatus1,
        currentStatus2: fd.currentStatus2,
        isOTC: fd.isOTC,
        productRating: fd.productRating,
      ...
      const url = editProductId
        ? updateProductPath(editProductId)
        : createProductPath;
      const method = editProductId ? "put" : "post";
      const response = await requestClient[method](url, payload, { timeout: 60000 });
      if (response.data.success) {
        show(
          `"${fd.brandName}" ${editProductId ? "updated" : "saved"} successfully!`,
        );
        onSuccess?.();
  ...
  const margin = (() => {
    const cost = n(fd.totalCostOfStores),
      sale = n(fd.totalSaleValue);
  ...
  const DISCOUNT_ROLES = [
    ...
    },
    {
      ...
      bg: "#fff3e0",
      discF: null,
      rateF: "wholesaleSaleRate",
      totalF: "totalWholesaleValue",
      icon: <LocalOfferIcon />,
      isWsr: true,
    },
    {
      ...
      color: "#0277bd",
      bg: "#e1f5fe",
      discF: "hospitalDiscount",
      rateF: "hospitalSaleRate",
      totalF: "hospitalTotalValue",
    ...
    },
    {
      ...
      color: "#2e7d32",
      bg: "#e8f5e9",
      discF: "pharmacyDiscount",
      rateF: "pharmacySaleRate",
      totalF: "pharmacyTotalValue",
      icon: <StorefrontIcon />,
    },
    {
      key: "vendor",
    ...
    },
    {
      ...
      color: "#4527a0",
      bg: "#ede7f6",
      discF: "manufacturerDiscount",
      rateF: "manufacturerRate",
      totalF: null,
      icon: <FactoryIcon />,
    },
  ];
        ...
        <Container maxWidth="xl">
          ...
          >
            <Box display="flex" alignItems="center" gap={2}>
              <Avatar
                sx={{ bgcolor: "rgba(255,255,255,.2)", width: 48, height: 48 }}
              >
                <InventoryIcon />
              </Avatar>
              <Box>
            ...
            <Box display="flex" alignItems="center" gap={2} flexWrap="wrap">
              <Chip
                icon={<AccessTimeIcon />}
                label={currentDateTime.toLocaleString()}
                sx={{
            ...
            <form onSubmit={handleSubmit}>
              {isBulkManufacturingDesk && (
                <Alert severity="info" sx={{ mb: 2, borderRadius: 3 }}>
                  Bulk-manufacturing products appear on the public website only
                  after you assign at least one section in
                  {" "}
                  <strong>Category, Sections &amp; Tags</strong>.
                </Alert>
              )}
              {/* 1. Media */}
              <Accordion
                ...
                >
                  <Box display="flex" alignItems="center" gap={1.5}>
                    <Avatar sx={{ bgcolor: "#e3f2fd", width: 32, height: 32 }}>
                      <PhotoCameraIcon color="primary" sx={{ fontSize: 18 }} />
                    </Avatar>
                    <Typography fontWeight={700}>Product Media</Typography>
                    {fd.images.length > 0 && (
                      <Chip
                        label={`${fd.images.length} image(s)`}
                        size="small"
                        color="success"
                      />
                    )}
                  ...
                  <Grid container spacing={2}>
                    <Grid item xs={12} md={7}>
                      <FileUploadArea
                        ...
                        accept="image/*"
                        multiple
                        onFilesSelect={(files) => set("images", files)}
                        icon={
                          <PhotoCameraIcon
                            sx={{ fontSize: 40, color: "#1565c0" }}
                          />
                        }
                        type="image"
                      />
                    ...
                    <Grid item xs={12} md={5}>
                      <FileUploadArea
                        ...
                        accept="video/*"
                        multiple={false}
                        onFilesSelect={(files) => set("video", files)}
                        icon={
                          <VideocamIcon
                            sx={{ fontSize: 40, color: "#1565c0" }}
                          />
                ...
                >
                  <Box display="flex" alignItems="center" gap={1.5}>
                    <Avatar sx={{ bgcolor: "#e8f5e9", width: 32, height: 32 }}>
                      <InfoIcon color="success" sx={{ fontSize: 18 }} />
                    </Avatar>
                ...
                <AccordionDetails sx={{ bgcolor: "#fff", pt: 0 }}>
                  <Grid container spacing={2}>
                    {[
                      ...
                      {
                        label: "Case Box Package",
                        field: "caseBoxPackage",
                        md: 3,
                      },
                    ...
                    <Grid item xs={12} md={6}>
                      ...
                      <Box display="flex" gap={1}>
                        <FormControl size="small" fullWidth>
                          ...
                          <Select
                            value={selectedInjType}
                            label="Select"
                            onChange={(e) => setSelectedInjType(e.target.value)}
                          >
                            {[
                              "Vials",
                              "Ampoules",
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (
                              !selectedInjType ||
                              injectionTypes.includes(selectedInjType)
                            )
                              return;
                            setInjectionTypes([
                              ...injectionTypes,
                      ...
                      <Box display="flex" gap={1}>
                        <FormControl size="small" fullWidth>
                          ...
                          <Select
                            ...
                            label="Select"
                            onChange={(e) => setSelectedTabType(e.target.value)}
                          >
                            {[
                              "Blisters",
                              "Strips",
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (
                              !selectedTabType ||
                              tabletTypes.includes(selectedTabType)
                            )
                              return;
                            setTabletTypes([...tabletTypes, selectedTabType]);
                            setSelectedTabType("");
                          }}
                        >
                          Add
                ...
                >
                  <Box display="flex" alignItems="center" gap={1.5}>
                    <Avatar sx={{ bgcolor: "#fce4ec", width: 32, height: 32 }}>
                      <CurrencyRupeeIcon color="error" sx={{ fontSize: 18 }} />
                    </Avatar>
                    ...
                    <Grid item xs={12}>
                      ...
                      >
                        ...
                        <Grid container spacing={2}>
                          {[
                            {
                              label: "Total Cost of Stores",
                              value: fd.totalCostOfStores,
                              helper: "AMP × Stocks",
                            },
                            {
                              label: "Total Actual Value",
                              value: fd.totalActualValue,
                              helper: "MRP × Stocks",
                            },
                            {
                              label: "B2C Sale Total",
                              value: fd.totalSaleValue,
                              helper: "PTR × Stocks",
                            },
              ...
              >
                ...
                >
                  <Box display="flex" alignItems="center" gap={1.5}>
                    <Avatar sx={{ bgcolor: "#ede7f6", width: 32, height: 32 }}>
                      <PercentIcon color="secondary" sx={{ fontSize: 18 }} />
                    </Avatar>
                    <Typography fontWeight={700}>
                      Discounts & Sales Rates (All Roles)
                    </Typography>
                  </Box>
                </AccordionSummary>
                <AccordionDetails sx={{ bgcolor: "#fff", pt: 0 }}>
                    ...
                    {DISCOUNT_ROLES.map(
                      ({
                        ...
                        <Paper
                          key={key}
                          variant="outlined"
                          sx={{ p: 2, borderRadius: 2, borderColor: color }}
                        >
                          <Box
                            display="flex"
                            alignItems="center"
                            ...
                            {isWsr ? (
                              <>
                                <Grid item xs={12} md={4}>
                                  <TextField
                                    fullWidth
                                    label="Wholesale Sale Rate (WSR) ₹"
                                    type="number"
                                    value={fd.wholesaleSaleRate}
                                    onChange={(e) =>
                                      set("wholesaleSaleRate", e.target.value)
                                    }
                                    InputProps={{
                                      startAdornment: (
                                        <InputAdornment position="start">
                                ...
                                <Grid item xs={12} md={4}>
                                  <TextField
                                    ...
                                    value={`₹ ${fd.totalWholesaleValue}`}
                                    InputProps={{ readOnly: true }}
                                    sx={{ bgcolor: bg }}
                                    helperText="WSR × Stocks"
                                  />
                                </Grid>
                                <Grid item xs={12} md={4}>
                                  {fd.wholesaleSaleRate && fd.mrp && (
                                    <Box
                                      sx={{
                                        p: 1.5,
                                        borderRadius: 2,
                                ...
                                {rateF && (
                                  <Grid item xs={12} md={3}>
                                    <TextField
                                      fullWidth
                                      label="Sale Rate"
                                      value={`₹ ${fd[rateF] || fmt(n(fd.mrp) * (1 - n(fd[discF]) / 100))}`}
                                      InputProps={{ readOnly: true }}
                                      sx={{ bgcolor: bg }}
                                    />
                                  </Grid>
                                ...
                                {key === "b2b" && (
                                  <Grid item xs={12} md={3}>
                                    <TextField
                                      fullWidth
                                      label="B2B Rate/unit"
                                      value={`₹ ${fmt(n(fd.mrp) * (1 - n(fd.b2bDiscount) / 100))}`}
                                      InputProps={{ readOnly: true }}
                                      sx={{ bgcolor: bg }}
                                    />
                                  </Grid>
                                ...
                                <Grid item xs={12} md={3}>
                                  ...
                                  >
                                    ...
                                    <Typography fontWeight={700} sx={{ color }}>
                                      ₹{" "}
                                      {fmt(n(fd.mrp) - n(fd[rateF] || fd.mrp))}
                                    </Typography>
                                  </Box>
                                </Grid>
                              </>
              ...
              >
                ...
                >
                  <Box display="flex" alignItems="center" gap={1.5}>
                    <Avatar sx={{ bgcolor: "#e0f2f1", width: 32, height: 32 }}>
                      <AccountBalanceIcon
                        color="primary"
                        sx={{ fontSize: 18 }}
                      />
              ...
              >
                ...
                >
                  <Box display="flex" alignItems="center" gap={1.5}>
                    <Avatar sx={{ bgcolor: "#fff8e1", width: 32, height: 32 }}>
                      <LocalOfferIcon color="warning" sx={{ fontSize: 18 }} />
                    </Avatar>
                    <Typography fontWeight={700}>
                      Offers, Schemes & Coupons
                    </Typography>
                    <Chip
                      label={`${fd.offersWithIcon.length} offers · ${coupons.length} coupons`}
                      size="small"
                      color="warning"
                    />
                  </Box>
                ...
                <AccordionDetails sx={{ bgcolor: "#fff", pt: 0 }}>
                  <Stack spacing={3}>
                    <OffersManager
                      offers={fd.offersWithIcon}
                      onChange={(v) => set("offersWithIcon", v)}
                    />
                    <Grid container spacing={2}>
                      <Grid item xs={12} md={6}>
                        <TextField
                          ...
                          value={fd.scheme1}
                          onChange={(e) => set("scheme1", e.target.value)}
                          InputProps={{
                            startAdornment: (
                              <InputAdornment position="start">
                                <LoyaltyIcon />
                      ...
                      <Grid item xs={12} md={6}>
                        <TextField
                          fullWidth
                          label="Scheme II"
                          placeholder="e.g. Extra 5% on bulk"
                          value={fd.scheme2}
                          onChange={(e) => set("scheme2", e.target.value)}
                          InputProps={{
                            startAdornment: (
                              <InputAdornment position="start">
                                <LoyaltyIcon />
                      ...
                      <Grid item xs={12}>
                        <TextField
                          ...
                          multiline
                          rows={2}
                          label="Additional Offers / Notes"
                          value={fd.additionalOffers}
                          onChange={(e) =>
                            set("additionalOffers", e.target.value)
                          }
                        />
                      </Grid>
                    </Grid>
              ...
              >
                ...
                >
                  <Box display="flex" alignItems="center" gap={1.5}>
                    <Avatar sx={{ bgcolor: "#fce4ec", width: 32, height: 32 }}>
                      <AutoFixHighIcon color="error" sx={{ fontSize: 18 }} />
                    </Avatar>
                    <Typography fontWeight={700}>
                      Variants, Combos & Specifications
                    </Typography>
                    <Chip
                      label={`${variants.length} variant · ${combos.length} combo · ${combinations.length} attr-combo · ${specifications.length} spec`}
                      size="small"
                    />
                  </Box>
                </AccordionSummary>
              ...
              >
                ...
                >
                  <Box display="flex" alignItems="center" gap={1.5}>
                    ...
                    <Typography fontWeight={700}>
                      Category, Sections & Tags
                    </Typography>
                    {sections.length > 0 && (
                      <Chip
                        label={`${sections.length} section(s)`}
                        size="small"
                        color="primary"
                      />
                    )}
        ...
        >
          {isSub ? `↳ ${c.title}` : c.title}
                      ...
                      <Grid item xs={12} md={6}>
                        <FormControl
                          fullWidth
                          disabled={!selectedMainCat || subCatLoading}
                        >
                          <InputLabel>
                            {subCatLoading
                              ? "Loading..."
                      ...
                      {!hideFranchiseZoneField && (
                        <Grid item xs={12}>
                          <FormControl fullWidth>
                            <InputLabel>
                              {franchiseZonesLoading
                                ? "Loading franchise zones..."
                                : "Franchise Fulfilment Zone"}
                            </InputLabel>
                            <Select
                              value={selectedFranchiseZone}
                              label="Franchise Fulfilment Zone"
                              onChange={(e) =>
                                setSelectedFranchiseZone(e.target.value)
                              }
                            >
                              <MenuItem value="">
                                <em>No franchise routing</em>
                              </MenuItem>
          ...
          <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5 }}>
            {selected.map((sKey) => {
              const sec = availableHomeSections.find(
                (s) =>
                  (s.key || s.slug || s.title?.toLowerCase().replace(/\s+/g, "-")) === sKey
              );
              return (
                <Chip
                  key={sKey}
...
<Divider />
                      ...
                      <Box display="flex" gap={1} mb={1}>
                        <TextField
                          ...
                          onKeyDown={(e) => {
                            ...
                            set("tags", [...fd.tags, tagInput.trim()]);
                            setTagInput("");
                          }}
                        />
                        <Button
              ...
              >
                ...
                >
                  <Box display="flex" alignItems="center" gap={1.5}>
                    <Avatar sx={{ bgcolor: "#e0f7fa", width: 32, height: 32 }}>
                      <DescriptionIcon color="info" sx={{ fontSize: 18 }} />
                    </Avatar>
                ...
                <AccordionDetails sx={{ bgcolor: "#fff", pt: 0 }}>
                  <Stack spacing={2}>
                    {[
                      {
                        label: "Product Description (shown in Description tab)",
                        field: "description",
                        rows: 5,
                      },
                      {
                        label: "More Information / Short Description",
              ...
              >
                ...
                >
                  <Box display="flex" alignItems="center" gap={1.5}>
                    <Avatar sx={{ bgcolor: "#e8f5e9", width: 32, height: 32 }}>
                      <PersonIcon color="success" sx={{ fontSize: 18 }} />
                    </Avatar>
                    <Typography fontWeight={700}>
                      Author Details & Marketer Info
                    </Typography>
                    {authors.length > 0 && (
                      <Chip
                        label={`${authors.length} author(s)`}
                        size="small"
                        color="success"
                      />
                    )}
              ...
              >
                ...
                >
                  <Box display="flex" alignItems="center" gap={1.5}>
                    <Avatar sx={{ bgcolor: "#fffde7", width: 32, height: 32 }}>
                  ...
                  <Stack spacing={3}>
                    <Box>
                      ...
                      >
                        <Rating
                          size="large"
                          precision={0.5}
                          value={parseFloat(fd.productRating) || 0}
                    ...
                    <Box>
                      ...
                      <TextField
                        ...
                        multiline
                        rows={3}
                        value={fd.topReviewFromIndia}
                        onChange={(e) =>
                          set("topReviewFromIndia", e.target.value)
                        }
                      />
                    </Box>
                    <Box>
              ...
              >
                ...
                >
                  <Box display="flex" alignItems="center" gap={1.5}>
                    <Avatar
                      sx={{
                        bgcolor: fd.isOTC ? "#e8f5e9" : "#ffebee",
                        width: 32,
                        height: 32,
                      }}
                    >
                      <WarningIcon
                        color={fd.isOTC ? "success" : "error"}
                        sx={{ fontSize: 18 }}
                      />
                    </Avatar>
                    <Typography fontWeight={700}>
                    ...
                    <Chip
                      label={fd.currentStatus1}
                      color={
                        fd.currentStatus1 === "active" ? "success" : "default"
                      }
                      size="small"
                    />
                    <Chip
                      label={fd.isOTC ? "OTC" : "Rx Required"}
                      color={fd.isOTC ? "success" : "error"}
                      size="small"
                    />
                  </Box>
                  ...
                  <Stack spacing={3}>
                    <Grid container spacing={2}>
                      <Grid item xs={12} md={6}>
                        <FormControl fullWidth>
                          ...
                          <Select
                            value={fd.currentStatus1}
                            label="Visibility Status"
                            onChange={(e) =>
                              set("currentStatus1", e.target.value)
                            }
                          >
                            ...
                            <MenuItem value="inactive">
                              <Chip label="Inactive — hidden" size="small" />
                            </MenuItem>
                          </Select>
                        </FormControl>
                      </Grid>
                      <Grid item xs={12} md={6}>
                        <FormControl fullWidth>
                          ...
                          <Select
                            value={fd.currentStatus2}
                            label="Display Mode"
                            onChange={(e) =>
                              set("currentStatus2", e.target.value)
                            }
                          >
                            <MenuItem value="appear">
                              Appear on storefront
                            </MenuItem>
                            <MenuItem value="disappear">
                    ...
                    <Paper
                      ...
                      sx={{
                        p: 2.5,
                        borderRadius: 2,
                        border: `2px solid ${fd.isOTC ? "#4caf50" : "#f44336"}`,
                      }}
                    >
                      <RadioGroup
                        row
                        value={String(fd.isOTC)}
                        onChange={(e) =>
                          set("isOTC", e.target.value === "true")
                        }
                      >
                        <FormControlLabel
                          value="true"
                          ...
                          label={
                            <Box>
                              ...
                              <Typography
                                ...
                                color="text.secondary"
                              >
                                Customer must upload prescription
                              </Typography>
                            </Box>
                          }
                        />
              ...
              >
                ...
                >
                  <Box display="flex" alignItems="center" gap={1.5}>
                    ...
                    <Chip
                      label={`${recommendations.length} rec · ${certifications.length} cert`}
                      size="small"
                      color="success"
                    />
                  </Box>
              ...
              >
                ...
                <Button
                  ...
                  onClick={() =>
                    window.confirm("Reset all form data?") &&
                ...
                <CardContent>
                  ...
                  <Grid container spacing={1.5}>
                    {[
                      ...
                      {
                        label: "Hospital",
                        value: `₹ ${fd.hospitalSaleRate}`,
                        color: "#80cbc4",
                      },
                      {
                        label: "Pharmacy",
                        value: `₹ ${fd.pharmacySaleRate}`,
                        color: "#a5d6a7",
                      },
                      {
                        label: "Vendor",
                      ...
                      {
                        label: "Franchise",
                        value: `₹ ${fd.franchiseRate}`,
                        color: "#ffcc80",
                      },
                      {
                        label: "Manufacturer",
                        value: `₹ ${fd.manufacturerRate}`,
                        color: "#b0bec5",
                      },
                      {
                        label: "Total Stocks",
                        value: fd.totalStocks || "0",
                        color: "white",
                      },
                      {
                        label: "Coupons",
                        value: `${coupons.length} active`,
                        color: "#f48fb1",
                      },
                      {
                        label: "Combos",
                        value: `${combos.length} bundle(s)`,
                        color: "#a5d6a7",
                      },
                      {
                        label: "Recs",
                        value: `${recommendations.length} rec · ${certifications.length} cert`,
                        color: "#90caf9",
                      },
                    ].map((item) => (
                      <Grid item xs={6} key={item.label}>
                        ...
                        >
                          ...
                          <Typography
                            fontWeight={700}
                            sx={{ color: item.color, fontSize: ".9rem" }}
                          >
                            {item.value}
                          </Typography>
                        </Box>
                          ...
                          {checks.map((c) => (
                            ...
                            >
                              ...
                              <Typography
                                variant="caption"
                                color={
                                  c.done ? "text.primary" : "text.secondary"
                                }
                              >
                                {c.label}
                              </Typography>
