...
const TYPE_BADGE = {
  ...
  "Internship": { bg: "#fef9c3", color: "#a16207" },
...
export default function JobsReports() {
  ...
  // ── ResizeObserver — measures real content width, not window width ─────────
  ...
  const filteredApps = useMemo(() => {
    return dateFilteredApps.filter((a) => {
      ...
      const matchType   = typeFilter   === "all" || a.jobType === typeFilter;
  ...
  }, [dateFilteredApps, statusFilter, typeFilter, deptFilter, searchFilter]);
  ...
  const uniqueTypes = [...new Set(apps.map((a) => a.jobType).filter(Boolean))];
  const uniqueDepts = [...new Set(apps.map((a) => a.department).filter(Boolean))];
  ...
  const exportApplicants = () => {
    exportCSV(
      filteredApps.map((a) => ({
        ...
        "Job Type":    a.jobType || "—",
        Location:      a.city ? `${a.city}, ${a.state}` : "—",
        Experience:    a.workExperience || "—",
        Qualification: a.qualification || "—",
        Status:        a.status || "pending",
        "Applied On":  fmt(a.createdAt),
      })),
      `jobs-applicants-${startMonth}-to-${endMonth}.csv`
    );
  };
  ...
  const exportSummary = () => {
    exportCSV(
      ...
      `jobs-summary-${startMonth}-to-${endMonth}.csv`
    );
  };
  ...
  const exportJobReport = () => {
    exportCSV(
      ...
      `jobs-positions-report-${startMonth}-to-${endMonth}.csv`
    );
  };
      ...
      <div ref={containerRef} style={s.page}>
        <div style={s.header}>
          <div>
            ...
            <p style={s.label}>Jobs & Careers Zone · Reports</p>
            <h1 style={{ ...s.title, fontSize: isMobile ? 17 : 21 }}>{job.title}</h1>
            <p style={s.sub}>{[job.department, job.location, job.jobType].filter(Boolean).join(" · ")}</p>
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            <button
              onClick={() => exportCSV(
                jApps.map((a) => ({
                  Name: a.fullName, Email: a.email, Phone: a.phone || "—",
                  "Job Type": a.jobType || "—", Experience: a.workExperience || "—",
                  Qualification: a.qualification || "—",
              ...
              return (
                <div key={st} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                  ...
                  <div style={{ width: "100%", height: 48, display: "flex", alignItems: "flex-end" }}>
                    <div style={{ width: "100%", height: `${Math.max(pct, 4)}%`, background: c.bar, borderRadius: "3px 3px 0 0", transition: "height 0.4s ease" }} />
                  </div>
        ...
        {isMobile ? (
              ...
              : jApps.map((a) => {
                  ...
                  return (
                    <div key={a._id} style={{ ...s.mCard, borderLeft: `3px solid ${c.bar}` }}>
                      ...
                      <p style={{ fontSize: 12, color: "#64748b", margin: "2px 0" }}>{a.jobType || "—"} · {a.workExperience || "—"}</p>
          ...
          <div style={s.tableWrap}>
            <table style={s.table}>
              <thead>
                <tr>
                  {["#", "Name", "Email", "Phone", "Job Type", "Experience", "Qualification", "Applied On", "Status"].map(
              ...
              <tbody>
                {jApps.length === 0
                  ? <tr><td colSpan={9} style={{ ...s.td, textAlign: "center", color: "#94a3b8", padding: 40 }}>No applicants found.</td></tr>
                  : jApps.map((a, i) => {
                      const st = a.status || "pending";
                      const c  = STAGE_COLORS[st];
                      return (
                        <tr key={a._id} style={{ ...s.tr, background: i % 2 === 0 ? "#fff" : "#f8fafc" }}>
                          ...
                          <td style={s.td}>
                            {a.jobType
                              ? <span style={{ ...s.chip, ...(TYPE_BADGE[a.jobType] || TYPE_BADGE.Other) }}>{a.jobType}</span>
                              : "—"}
                          </td>
                          <td style={s.td}>{a.workExperience || "—"}</td>
  ...
  return (
    <div ref={containerRef} style={s.page}>
      ...
      <div style={s.header}>
        <div>
          <p style={s.label}>Jobs & Careers Zone</p>
          <h1 style={{ ...s.title, fontSize: isMobile ? 18 : 24 }}>Recruitment Reports</h1>
          <p style={s.sub}>Detailed analytics & applicant records · {startMonth} to {endMonth}</p>
        </div>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <input type="month" value={startMonth} onChange={(e) => setStartMonth(e.target.value)} style={s.monthInput} />
          <span style={{ fontSize: 12, color: "#94a3b8" }}>to</span>
          <input type="month" value={endMonth} onChange={(e) => setEndMonth(e.target.value)} style={s.monthInput} />
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <span style={{ fontSize: 12, fontWeight: 600, color: "#64748b" }}>Status:</span>
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} style={s.selectInput}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} style={s.selectInput}>
            <option value="all">All</option>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <select value={deptFilter} onChange={(e) => setDeptFilter(e.target.value)} style={s.selectInput}>
            <option value="all">All</option>
        ...
        <div style={{ flex: 1, minWidth: 180 }}>
          <input
            type="text" placeholder="Search applicant / role..."
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 22, borderBottom: "2px solid #e2e8f0" }}>
        {[
          ...
          <button
            ...
            style={{
              padding: "9px 18px", fontSize: isMobile ? 12 : 13,
          ...
          {view === "overview" && (
            <>
              ...
              <div style={{
                ...
                {[
                  ...
                  <div key={c.label} style={s.statCard}>
                    <p style={{ fontSize: isMobile ? 20 : 26, fontWeight: 700, color: c.accent, margin: "0 0 4px" }}>{c.val}</p>
              ...
              <div style={{
                ...
                {[
                  ...
                  <div key={r.label} style={{ ...s.reportBox, background: r.bg, borderColor: "transparent" }}>
                    <p style={{ fontSize: 11, fontWeight: 600, color: "#64748b", margin: "0 0 6px", textTransform: "uppercase", letterSpacing: "0.06em" }}>{r.label}</p>
              ...
              <div style={{
                      ...
                      return (
                        <div key={m.key} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
                          ...
                          <div style={{ width: "100%", height: 56, display: "flex", alignItems: "flex-end" }}>
                            <div style={{ width: "100%", height: `${Math.max(pct, 3)}%`, background: "linear-gradient(to top, #0077a3, #38bdf8)", borderRadius: "3px 3px 0 0", transition: "height 0.4s ease" }} />
                          </div>
                ...
                {/* Department breakdown */}
                <div style={s.reportBox}>
                  <p style={s.boxTitle}>Applications by Department</p>
                  <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 4 }}>
                    {deptData.length === 0
                      ? <span style={{ fontSize: 13, color: "#94a3b8" }}>No data</span>
                      : deptData.slice(0, 7).map(({ label, value }) => {
                          const pct = dateFilteredApps.length > 0 ? Math.round((value / dateFilteredApps.length) * 100) : 0;
                          return (
                            <div key={label}>
                              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                                <span style={{ fontSize: 12, color: "#334155", fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "70%" }}>{label}</span>
                                <span style={{ fontSize: 12, fontWeight: 700, color: "#334155" }}>{value} <span style={{ color: "#94a3b8", fontWeight: 400 }}>({pct}%)</span></span>
                              </div>
                              <div style={{ height: 6, background: "#f1f5f9", borderRadius: 4, overflow: "hidden" }}>
                                <div style={{ height: "100%", width: `${pct}%`, background: "#0077a3", borderRadius: 4, opacity: 0.55, transition: "width 0.5s ease" }} />
                              </div>
                            </div>
                          );
                        })}
                ...
                {/* Job type breakdown */}
                {typeData.length > 0 && (
                  <div style={{ ...s.reportBox, gridColumn: isMobile ? "auto" : "1 / -1" }}>
                    <p style={s.boxTitle}>Applications by Job Type</p>
                    <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                      {typeData.map(({ label, value }) => {
                        const tb  = TYPE_BADGE[label] || TYPE_BADGE.Other;
                        const pct = dateFilteredApps.length > 0 ? Math.round((value / dateFilteredApps.length) * 100) : 0;
                        return (
                          <div key={label} style={{ background: tb.bg, borderRadius: 10, padding: "12px 18px", textAlign: "center", minWidth: 100 }}>
                            <p style={{ fontSize: 22, fontWeight: 700, color: tb.color, margin: 0 }}>{value}</p>
                            <p style={{ fontSize: 11, color: tb.color, margin: "3px 0 0", fontWeight: 600 }}>{label}</p>
                            <p style={{ fontSize: 10, color: "#94a3b8", margin: "2px 0 0" }}>{pct}%</p>
                          </div>
                        );
                      })}
                    </div>
              ...
              <div style={{
                ...
                {grouped.length === 0 ? (
                  ...
                  grouped.map(({ job, apps: jApps, counts, total }) => {
                    const tb = TYPE_BADGE[job.jobType] || TYPE_BADGE.Other;
                    return (
                      <div key={job._id} style={s.card}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10, marginBottom: 14 }}>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ display: "flex", gap: 7, flexWrap: "wrap", marginBottom: 6 }}>
                              {job.jobType && <span style={{ ...s.chip, background: tb.bg, color: tb.color }}>{job.jobType}</span>}
                              {job.department && <span style={{ ...s.chip, background: "#f1f5f9", color: "#475569" }}>{job.department}</span>}
                            </div>
                            ...
                            <p style={{ fontSize: 12, color: "#64748b", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                              {[job.location, job.experience].filter(Boolean).join(" · ")}
                            </p>
                          </div>
              ...
              {uncategorized.length > 0 && (
                <div style={{ marginTop: 28 }}>
                  ...
                  <div style={s.tableWrap}>
                    <table style={s.table}>
                      <thead>
                        <tr>{["Name", "Applying For", "Job Type", "Email", "Applied On", "Status"].map((h) => <th key={h} style={s.th}>{h}</th>)}</tr>
                      </thead>
                      <tbody>
              ...
              {isMobile ? (
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {filteredApps.length === 0
                    ? <div style={s.empty}>No applicants match the selected filters.</div>
                    : filteredApps.map((a) => {
                        const st = a.status || "pending";
                        const c  = STAGE_COLORS[st];
                        return (
                          <div key={a._id} style={{ ...s.mCard, borderLeft: `3px solid ${c.bar}` }}>
                            ...
                            <p style={{ fontSize: 12, color: "#0077a3", fontWeight: 600, margin: "2px 0" }}>{a.applyingFor}</p>
                            <p style={{ fontSize: 12, color: "#64748b", margin: "2px 0" }}>{a.jobType || "—"} · {a.workExperience || "—"}</p>
                            <p style={{ fontSize: 12, color: "#94a3b8", margin: "2px 0" }}>{a.email}</p>
                            <p style={{ fontSize: 11, color: "#94a3b8", marginTop: 4 }}>{fmt(a.createdAt)}</p>
                          </div>
                ...
                <div style={s.tableWrap}>
                  <table style={s.table}>
                    <thead>
                      <tr>
                        {["#", "Name", "Email", "Applying For", "Job Type", "Experience", "Qualification", "Applied On", "Status"].map(
                          (h) => <th key={h} style={s.th}>{h}</th>
                        )}
                      </tr>
                    </thead>
