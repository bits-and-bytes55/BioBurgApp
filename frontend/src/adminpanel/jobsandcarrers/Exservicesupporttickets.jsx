...
export default function ExServiceSupportTickets() {
  ...
  const fetchStats = useCallback(async () => {
    try {
      const r = await axios.get(`${BASE_API}/api/exservice-support/stats`, { headers });
  ...
  const fetchTickets = useCallback(async () => {
    ...
    try {
      ...
      const r = await axios.get(`${BASE_API}/api/exservice-support`, { headers, params });
      if (r.data.success) {
  ...
  const handleUpdate = async () => {
    ...
    setUpdating(true);
    try {
      await axios.patch(`${BASE_API}/api/exservice-support/${selected._id}`,
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${BASE_API}/api/exservice-support/${id}`, { headers });
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: "#64748b", margin: "4px 0 0" }}>
              {isMobile ? "Veterans' support tickets" : "All support tickets submitted by veterans from the ExService Careers page"}
            </p>
          </div>
          {isMobile && (
        ...
        {stats && (
          <div style={{
            ...
            {[
              ...
              { label: "In Progress", val: stats.in_progress, bg: "#eff6ff",  color: "#1d4ed8" },
              { label: "Resolved",    val: stats.resolved,    bg: "#f0fdf4",  color: "#15803d" },
              { label: "Closed",      val: stats.closed,      bg: "#f1f5f9",  color: "#475569" },
            ].map((st, i) => (
              // On mobile hide "In Progress" and "Closed" to keep 3-col clean
              <div key={st.label} style={{
                ...
                <p style={{ fontSize: isMobile ? 20 : 26, fontWeight: 700, color: st.color, margin: 0 }}>{st.val}</p>
                <p style={{ fontSize: isMobile ? 9 : 11, fontWeight: 700, color: "#94a3b8", margin: "3px 0 0", textTransform: "uppercase", letterSpacing: "0.05em" }}>
                  {isMobile && st.label === "In Progress" ? "Active" : st.label}
                </p>
              </div>
            ))}
          </div>
                ...
                return (
                  <div key={t._id} className="tcard"
                    style={{
                      ...
                      border: "1px solid #e8eef4", boxShadow: "0 2px 10px rgba(0,0,0,.05)",
      ...
      {selected && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,.65)", backdropFilter: "blur(4px)", zIndex: 1000, display: "flex", alignItems: isMobile ? "flex-end" : "center", justifyContent: "center", padding: isMobile ? 0 : 20 }}>
          <div style={{
            ...
            {/* Modal Header */}
            <div style={{ background: "linear-gradient(135deg,#1a1a2e,#0f3460)", borderRadius: isMobile ? "20px 20px 0 0" : "20px 20px 0 0", padding: isMobile ? "18px 20px" : "22px 28px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ minWidth: 0 }}>
                <p style={{ fontSize: 10, color: "#60a5fa", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.15em", margin: "0 0 3px" }}>
            ...
            <div style={{ padding: isMobile ? "18px 16px 24px" : "24px 28px 28px" }}>
              ...
              <div style={{ marginBottom: 18 }}>
                <p style={{ fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.07em", margin: "0 0 8px" }}>Message from Veteran</p>
              ...
              <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", flexWrap: isMobile ? "wrap" : "nowrap" }}>
                ...
                <button onClick={handleUpdate} disabled={updating}
                  style={{ flex: isMobile ? "1 0 100%" : "unset", padding: "10px 22px", background: updating ? "#94a3b8" : "linear-gradient(135deg,#0077a3,#0f3460)", color: "#fff", border: "none", borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: updating ? "wait" : "pointer" }}>
...
const EmptyState = () => (
  <div style={{ textAlign: "center", padding: 60 }}>
    ...
    <p style={{ color: "#94a3b8", fontSize: 13, marginTop: 4 }}>Try adjusting filters or wait for veterans to submit tickets</p>
  </div>
);