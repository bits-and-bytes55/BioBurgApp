...
const exportOrdersToCsv = (orders) => {
  ...
  link.download = `manufacturer-orders-${new Date().toISOString().slice(0, 10)}.csv`;
  link.click();
...
export default function ManufacturerOrders() {
  ...
  const fetchOrders = async (nextFilters = filters) => {
    try {
      setLoading(true);
      setError("");
      const response = await manufacturerApi.get("/manufacturer/orders", {
        params: nextFilters,
      });
      ...
      console.error("Manufacturer orders fetch error:", fetchError);
      setError(
        fetchError.response?.data?.message ||
          "Unable to load manufacturer-owned website orders.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const fetchOrderDetail = async (orderId, force = false) => {
    ...
    try {
      ...
      const response = await manufacturerApi.get(`/manufacturer/orders/${orderId}`);
      ...
      console.error("Manufacturer order detail error:", detailError);
      setError(
  ...
  const updateOrderStatus = async (orderId, nextStatus) => {
    try {
      ...
      await manufacturerApi.patch(`/manufacturer/orders/${orderId}/status`, {
        status: nextStatus,
      });
      ...
      console.error("Manufacturer order status update error:", updateError);
      setError(
  ...
  if (loading) {
    return <ConsoleLoading label="Loading manufacturer-owned website orders..." />;
  }
  ...
  return (
    <ConsolePage>
      <div className="grid gap-6">
        <ConsoleHeader
          eyebrow="Manufacturer Order Desk"
          title="Website orders routed to your manufacturer-owned catalog."
          description="Every order in this ledger belongs to products published under your manufacturer account. Review customer demand, move fulfilment forward, and export the current result set without touching franchise or bulk-owned traffic."
          badges={
            <>
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1.55fr)_340px]">
          ...
          >
            {orders.length ? (
                                  ...
                                  <div className="rounded-[24px] border border-white/[0.06] bg-white/[0.02] p-4">
                                    ...
                                    {nextStatuses.length ? (
                                      ...
                                      <div className="mt-4 text-sm leading-6 text-slate-500">
                                        No further manufacturer-side status movement is
              ...
              <ConsoleEmptyState
                title="No manufacturer-owned website orders found"
                text="Once customers place orders for your published manufacturer products, they will appear here."
              />
            )}
          </ConsolePanel>
          ...
          <div className="grid gap-6">
            ...
            >
              <div className="grid gap-3 text-sm leading-6 text-slate-500">
                <p>
                  Only website orders linked to your manufacturer-owned products
                  show up here.
                </p>
                <p>
                ...
                <p>
                  Use the status controls to move genuine manufacturer-owned
                  fulfilment forward without touching franchise or bulk orders.
                </p>
              </div>
            </ConsolePanel>
          </div>
