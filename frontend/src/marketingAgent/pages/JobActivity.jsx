...
const PLACE_NAME_LABEL = {
  ...
  Clinic: 'Clinic Name',
  Pharmacy: 'Pharmacy / Medical Store Name',
...
const PLACE_PLACEHOLDER = {
  ...
  Clinic: 'e.g. Dr. Sharma Clinic',
  Pharmacy: 'e.g. Sunrise Pharmacy',
  Office: 'e.g. Bioburg Head Office',
  Shop: 'e.g. Health & Wellness Store',
  Factory: 'e.g. ABC Pharmaceuticals',
  Hotel: 'e.g. Taj Hotel',
  School: 'e.g. DPS School',
  Other: 'Enter the place name'
}
...
export default function JobActivity() {
  ...
  // On mount: restore localStorage immediately, then verify backend
  useEffect(() => {
    const boot = async () => {
      ...
      try {
        const res = await fetch(`${API}/agent/job-status`, { headers: authHeaders() })
  ...
  // Timer — reads Date.now() so switching tabs doesn't break it
  useEffect(() => {
  ...
  const handleStart = async () => {
    ...
    setSubmitting(true)
    try {
      const res = await fetch(`${API}/agent/start-job`, {
        method: 'POST', headers: authHeaders(),
        body: JSON.stringify({ latitude: 0, longitude: 0, locationAccuracy: 10, state: '', district: '', area: '', address: '', startKm: parseFloat(startKm) })
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.message || 'Failed to start job')
      const t = Date.now()
      setJobStartTime(t); setJobStatus('active'); setElapsed(0)
      setTotalPausedMs(0); setVisits([])
      persist({ jobStatus: 'active', jobStartTime: t, totalPausedMs: 0, pauseStart: null, startKm, currentKm: '', visits: [] })
      setSnack({ open: true, msg: 'Job started! Have a productive day.', severity: 'success' })
    } catch (e) { setSnack({ open: true, msg: e.message, severity: 'error' }) }
    setSubmitting(false)
  }
  ...
  const handlePause = () => {
    const ps = Date.now()
    setPauseStart(ps); setJobStatus('paused')
  ...
  const handleResume = () => {
    ...
    setTotalPausedMs(newPaused); setPauseStart(null); setJobStatus('active')
  ...
  const handleEnd = async () => {
    ...
    setSubmitting(true)
    try {
      const res = await fetch(`${API}/agent/close-job`, {
        method: 'POST', headers: authHeaders(),
      ...
      clearInterval(timerRef.current)
      setJobEndTime(Date.now()); setJobStatus('ended')
  ...
  const handleAddVisit = () => {
    ...
    setVisitForm(EMPTY_VISIT); setVisitDialog(false)
  }
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: '#f8fafc', p: { xs: 2, md: 3 } }}>
      ...
      <Box sx={{ mb: 3, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          {statusBadge()}
          <Typography sx={{ fontFamily: 'monospace', fontSize: 13, color: '#475569', bgcolor: '#f1f5f9',
            px: 2, py: 0.75, borderRadius: 2, border: '1px solid #e2e8f0' }}>
      ...
      <Paper elevation={0} sx={{ border: '1px solid #e2e8f0', borderRadius: 3, p: 2.5, mb: 3, bgcolor: 'white' }}>
        ...
        <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          <TextField label="Start Reading (km)" size="small" type="number"
            value={startKm} disabled={jobStatus !== 'idle'}
            onChange={e => { setStartKm(e.target.value); persist({ startKm: e.target.value }) }}
            sx={{ width: 180 }} />
          <TextField label="Current Reading (km)" size="small" type="number"
      ...
      <Paper elevation={0} sx={{ border: '1px solid #e2e8f0', borderRadius: 3, p: 3, mb: 3, bgcolor: 'white' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 3 }}>
          ...
          <Box sx={{ display: 'flex', gap: 1.5, flexWrap: 'wrap' }}>
            {jobStatus === 'idle' && (
              <Button variant="contained" onClick={handleStart} disabled={submitting}
                sx={{ bgcolor: '#16a34a', '&:hover': { bgcolor: '#15803d' }, px: 3.5, py: 1.2,
                  borderRadius: 2, fontWeight: 700, fontSize: 13, boxShadow: '0 4px 14px rgba(22,163,74,0.35)' }}>
                ...
                {jobStatus === 'active'
                  ? <Button variant="outlined" onClick={handlePause}
                      sx={{ borderColor: '#d97706', color: '#d97706', '&:hover': { bgcolor: '#fffbeb' },
                        px: 3, py: 1.2, borderRadius: 2, fontWeight: 700, fontSize: 13 }}>
                      PAUSE
                    </Button>
                  : <Button variant="contained" onClick={handleResume}
                      sx={{ bgcolor: '#16a34a', '&:hover': { bgcolor: '#15803d' }, px: 3, py: 1.2,
                        borderRadius: 2, fontWeight: 700, fontSize: 13 }}>
                      RESUME
                    </Button>
                }
                <Button variant="outlined" onClick={() => setVisitDialog(true)}
                  sx={{ borderColor: '#1d4ed8', color: '#1d4ed8', '&:hover': { bgcolor: '#eff6ff' },
                    px: 3, py: 1.2, borderRadius: 2, fontWeight: 700, fontSize: 13 }}>
                  LOG VISIT
                </Button>
                <Button variant="contained" onClick={handleEnd} disabled={submitting}
                  sx={{ bgcolor: '#dc2626', '&:hover': { bgcolor: '#b91c1c' }, px: 3, py: 1.2,
                    borderRadius: 2, fontWeight: 700, fontSize: 13 }}>
      ...
      <Paper elevation={0} sx={{ border: '1px solid #e2e8f0', borderRadius: 3, overflow: 'hidden', bgcolor: 'white' }}>
        ...
        {visits.length === 0 ? (
          <Box sx={{ py: 6, textAlign: 'center' }}>
            <Typography sx={{ color: '#94a3b8', fontSize: 14 }}>No visits logged yet. Start your job and log visits as you go.</Typography>
          </Box>
        ) : (
      ...
      <Dialog open={visitDialog} onClose={() => { setVisitDialog(false); setVisitForm(EMPTY_VISIT) }}
        ...
        <DialogContent sx={{ pt: 3 }}>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
            ...
            {/* Step 2 — specific name, label & placeholder change with type */}
            ...
            {/* Extra field only when "Other" */}
            {visitForm.type === 'Other' && (
              <TextField fullWidth size="small"
                label="Describe the type of place"
                placeholder="e.g. Diagnostic Centre, NGO, Government Office..."
                value={visitForm.customType}
                onChange={e => setVisitForm(f => ({ ...f, customType: e.target.value }))} />
            )}

            <Box sx={{ display: 'flex', gap: 2 }}>
              <TextField fullWidth label="From Location" size="small" placeholder="e.g. Sector 14, Gurgaon"
                value={visitForm.fromLocation}
                onChange={e => setVisitForm(f => ({ ...f, fromLocation: e.target.value }))} />
              <TextField fullWidth label="To Location" size="small" placeholder="e.g. Civil Lines, Delhi"
      ...
      <Dialog open={summaryDialog} onClose={() => setSummaryDialog(false)} maxWidth="sm" fullWidth
        ...
        <DialogContent sx={{ pt: 3 }}>
          ...
          <Typography sx={{ fontWeight: 700, fontSize: 13, color: '#475569', mb: 1.5, letterSpacing: '0.05em' }}>
            LEAD BREAKDOWN
          </Typography>
